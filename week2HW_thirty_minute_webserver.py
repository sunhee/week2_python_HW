...
defaults = ['block115392-nzs.blueboxgrid.com', '8080']
# added /.py : 'x-application/py'/ for week2HW
...
#print current time modified by SJ
...
def get_content(uri):
    ...
    try:
        ...
        if os.path.isfile(path):
            #run ptint_time.py script and returns it's output (week2 HW)
            #look for python files
            if get_mime(uri) == 'x-application/py':
		print 'python file found, running'
		from subprocess import Popen, PIPE
		#command 'python print_time.py' to run print_time.py
		cmd = 'python print_time.py'
		p = Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True)
		#save output into stdout
		stdout = p.communicate()
		#returns it's output when clicked on the print_time.py in html
                return (200, 'text/html', stdout)
            else:
